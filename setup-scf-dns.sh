...
wait_for_lb() {
  ...
  set +e
  status=$(kubectl --namespace scf get svc "${svc_name}" -o jsonpath='{.status.loadBalancer.ingress[0].ip}' 2> /dev/null)
  set -e
...
wait_for_lb diego-ssh-ssh-proxy
wait_for_lb router-gorouter
wait_for_lb tcp-router-tcp-router

set +e
clear_ip "ssh.${SUBDOMAIN}"
clear_ip "tcp.${SUBDOMAIN}"
clear_ip "${SUBDOMAIN}"
clear_ip "*.${SUBDOMAIN}"
set -e

SSH_IP="$(get_lb diego-ssh-ssh-proxy)"
SCF_IP="$(get_lb router-gorouter)"
TCP_IP="$(get_lb tcp-router-tcp-router)"

set_ip "ssh.${SUBDOMAIN}" "${SSH_IP}"
set_ip "tcp.${SUBDOMAIN}" "${TCP_IP}"
set_ip "${SUBDOMAIN}" "${SCF_IP}"
set_ip "*.${SUBDOMAIN}" "${SCF_IP}"
